...
def timeIsOk(minTimeStart, maxTimeStop):
    return datetime.datetime.now() >= minTimeStart and datetime.datetime.now() <= maxTimeStop
    
def doStuffInEveryCity(session, data):
    for city in data:
        print("Setting location to " + city)
        session.set_custom_location(latitude=data[city]['latitude'], longitude=data[city]['longitude'])
        sleep(1)
        session.like(amount=random.randint(50, 100), ratio="69.0%", sleep=0.5)
        #session.superlike(amount=1)
        waitTime = random.randint(50,100)
        print("Wait " + str(waitTime) + " minutes for next swiping round.")
        sleep(waitTime *60)
        session.like(amount=random.randint(50, 100), ratio="69.0%", sleep=0.8)
        waitTime = random.randint(50,100)
        print("Wait " + str(waitTime) + " minutes to change location.")
        sleep(waitTime *60)


def doRestStuff(session):
...
if __name__ == "__main__":
    ...
    minTimeStart = now.replace(hour=7, minute=0, second=0, microsecond=0)
    ...
    while True:
        if (timeIsOk(minTimeStart, maxTimeStop)):
            doStuffInEveryCity(session, data)
            doRestStuff(session)
        else:
            print("Now is not the right time for the bot")
            timeToSleepSeconds = 60
            sleep(timeToSleepSeconds * 10)
